...
class LRMInferrer:
    def __init__(self, model_name: str , cache_dir='./.cache'):
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

        start_time = time.time()
        _checkpoint = self._load_checkpoint(model_name , cache_dir)
    ...
    def _default_render_cameras(self, batch_size: int = 1):
        ...
        render_camera_intrinsics = self._default_intrinsics().unsqueeze(0).repeat(render_camera_extrinsics.shape[0], 1,
                                                                                  1)
    ...
    def infer_single(self, image: torch.Tensor, render_size: int, mesh_size: int, export_video: bool,
                     export_mesh: bool):
        # image: [1, C_img, H_img, W_img]
        mesh_thres = 3.0
        chunk_size = 2
        batch_size = 1
        print("Going into source camera")
        source_camera = self._default_source_camera(batch_size).to(self.device)
        print("Going into render cameras")
        render_cameras = self._default_render_cameras(batch_size).to(self.device)

        with torch.no_grad():
            print("Going into forward planes")
            planes = self.model.forward_planes(image, source_camera)
            print("Going into forward synthesizer")
            results = {}
            ...
            if export_mesh:
                ...
                print("Going into marching cubes")
                ...
                vtx_colors = self.model.synthesizer.forward_points(planes, vtx_tensor)['rgb'].squeeze(
                    0).cpu().numpy()  # (0, 1)
    ...
    def infer(self, source_image: str, dump_path: str, source_size: int, render_size: int, mesh_size: int,
        ...
        end_time = time.time()
        elapsed_time = end_time - start_time
        print(f"Video saving time: {elapsed_time} seconds")
        # dump mesh
        start_time = time.time()
        if 'mesh' in results:
            mesh = results['mesh']
            # save ply format mesh
            glb_path = os.path.join(dump_path, f'{uid}.glb')
        ...
        end_time = time.time()
        elapsed_time = end_time - start_time
        print(f"Mesh saving time: {elapsed_time} seconds")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source_image', type=str, default='./../assets/sample_input/Testing image.jpg')
